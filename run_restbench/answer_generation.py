'''
Generate data for multiple categories and multiple tools
'''
import sys
import traceback
sys.path.append("../")
...
def api_json_to_openai_json(api_json,standard_tool_name):
    ...
    # Parameter type mapping to prevent errors when passing RapidAPI parameters to OpenAI functions
    map_type = {
        "NUMBER": "integer",
        "STRING": "string",
        "BOOLEAN": "boolean"
    ...
    templete["name"] = templete["name"][-64:] #Last 64 letters
    ...
    if "required_parameters" in api_json.keys() and len(api_json["required_parameters"]) > 0:
        for para in api_json["required_parameters"]:
            ...
            else: # All other parameter types are defaulted to string
                param_type = "string"
            prompt = {
                "type":param_type,
...
def method_converter(backbone_model,method,env,process_id,single_chain_max_step=24,max_query_count=60):
    ...
    if method.startswith("CoT"): #Single simulation UCT is CoT
...
from restbench_env import restbench_env
...
def run(method, backbone_model, query_id, data_dict, query, output_dir_path,tool_des,process_id=0):
    try:
        ...
        env = restbench_env(query_id, output_dir_path,process_id=process_id)
...
def contain(candidate_list,white_list):
    '''
    Ensure all items in candidate_list are present in white_list
    '''
    output = []
...
def main(query_dir, answer_dir, method,backbone_model):
    ...
    for k,idx in tqdm(enumerate(idx_list)):
        ...
        with open(os.path.join(env_datasets_path), "r", encoding='utf-8') as fr:
            ...
            query = f"{idx}.{json_data[idx]['query']}"

        task_list.append((method, backbone_model, idx, {}, query, answer_dir, ""))
...
if __name__ == "__main__":
    try:
        ...
        q=queue.Queue(10000000)  #Create a first-in-first-out queue
        starting_time = time.time()
        for task in task_list: #Shared task list, all threads consume
            q.put(task)
